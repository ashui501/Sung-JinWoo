...
import re
import traceback
import SungJinWoo.modules.sql.users_sql as sql
...
from SungJinWoo.modules import ALL_MODULES
from SungJinWoo.modules.helper_funcs.chat_status import is_user_admin
from SungJinWoo.modules.helper_funcs.alternate import typing_action
from SungJinWoo.modules.helper_funcs.misc import paginate_modules
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("SungJinWoo.modules." + module_name)
...
def SungJinWoo_callback_data(update, context):
    query = update.callback_query
    uptime = get_readable_time((time.time() - StartTime))
    if query.data == "SungJinWoo_":
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                     InlineKeyboardButton(text="⫷", callback_data="SungJinWoo_prev"),
                    InlineKeyboardButton(text="Back", callback_data="SungJinWoo_back"),
                     InlineKeyboardButton(text="⫸", callback_data="SungJinWoo_next")
                 ]
                ]
            ),
        )
    elif query.data == "SungJinWoo_back":
        first_name = update.effective_user.first_name
        query.message.edit_text(
                PM_START_TEXT.format(
                    escape_markdown(context.bot.first_name),
...
def main():
    ...
    data_callback_handler = CallbackQueryHandler(SungJinWoo_callback_data, pattern=r"SungJinWoo_", run_async=True)
    ...
    if WEBHOOK:
        ...
        LOGGER.info(f"SungJinWoo started, Using long polling. | SUPPORT: [@{SUPPORT_CHAT}]")
